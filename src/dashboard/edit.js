        ...
        <div className="max-w-[1600px] md:min-w-[500px] mx-auto bg-white">
            <div className="flex flex-col items-center w-full py-20">
                ...
                <div className="flex items-center gap-4">
                    <img className="bg-gray-100 p-3 rounded-[4px]" src={profileImg} alt="" onClick={() => { setIsOpen(!modalIsOpen) }} />
                    <img className="bg-gray-100 p-3 rounded-[4px]" src={settingsImg} alt="" onClick={setFilterModalCallback} />
                </div>

                <div className="flex items-center justify-center gap-6 mt-6 mb-4 lg:justify-start lg:gap-14 md:mb-8">
                    <div className="flex flex-col items-center justify-center">
                        <h2 className="font-semibold text-[20px] md:text-[28px] text-gray-800 font-MontserratBold">{numFormatter(user?.followers ? user?.followers : 0)}</h2>
                        <p className="text-sm font-normal opacity-90 font-MontserratLight">Followers</p>
                    </div>

                    <div className="flex flex-col items-center justify-center">
                        <h2 className="font-semibold text-[20px] md:text-[28px] text-gray-800 font-MontserratBold">{numFormatter(user?.following ? user?.following : 0)}</h2>
                        <p className="text-sm font-normal opacity-90 font-MontserratLight">Following</p>
                    </div>

                    <div className="flex flex-col items-center justify-center">
                        <h2 className="font-semibold text-[20px] md:text-[28px] text-gray-800 font-MontserratBold">{numFormatter(user?.posts ? user?.posts : 0)}</h2>
