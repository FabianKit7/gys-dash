...
axios.defaults.headers.post["x-access-key"] =
  "e1GKaU1YPsJNZlY1qTyj9i4J4yTIM7r1";
axios.defaults.headers.post["x-lama-reqid"] =
  "e1GKaU1YPsJNZlY1qTyj9i4J4yTIM7r1";
...
const SettingsModal = (props) => {
  ...
  const handleSave = async () => {
    setLoading(true);
    var d = { instagramPassword, userMode: mode, status: "new" };
        ...
        <div className="flex flex-col xl:flex-row justify-between gap-2 p-7 md:p-10">
          <div className="flex flex-col justify-center md:justify-start items-center md:mt-[39px]">
            <div className="relative w-[100px] h-[100px] md:w-[140px] md:h-[140px] rounded-full">
              <img
                className="h-full w-full mb-1 rounded-full"
                src={user?.profile_pic_url}
                alt=""
              />
              <div className="w-5 h-5 rounded-full bg-green-600 absolute bottom-[10px] right-[10px]"></div>
            </div>
            <h2 className="font-bold text-[#757575] text-[24px] my-1 font-MontserratBold">
              {user?.full_name}
            </h2>
            <h2 className="font-bold text-black text-[20px] my-1 font-MontserratBold">
              @{user?.username}
            </h2>
            ...
            <p className="font-normal text-xs md:text-sm font-MontserratLight opacity-90 mt-1">
              <span className="">
                Your password is 100% protected and encrypted.
              </span>{" "}
          ...
          <div className="w-full">
            <div className="flex flex-col gap-5">
              <div className="xl:mb-[10px] mt-4 xl:mt-0 text-[18px] xl:font-bold xl:font-MontserratBold">
              ...
              <div
                className={`${
                  mode === "auto"
                    ? "border-[4px] border-black h-[230px] md:h-[110px] lg:h-[200px]"
                    : "h-[52px]"
                } overflow-hidden py-[14px] md:py-5 px-[14px] md:px-10 cursor-pointer shadow-[0_0_3px_#00000040] rounded-[10px] flex items-center gap-2 md:gap-5`}
                style={{
              ...
              >
                ...
                <div className="">
                  <h3
                    ...
                    } font-bold font-MontserratBold`}
                  >
                    Auto Mode
                  </h3>
                  <p
                    className={`${
                      mode === "auto" ? "opacity-100" : "opacity-0 max-h-0"
                    } text-[12px] lg:text-base font-MontserratRegular`}
                    style={{
              ...
              <div
                className={`${
                  mode === "follow"
                    ? "border-[4px] border-black h-[270px] sm:h-[200px] md:h-[110px] lg:h-[200px]"
                    : "h-[52px]"
                } overflow-hidden py-[14px] md:py-5 px-[14px] md:px-10 cursor-pointer shadow-[0_0_3px_#00000040] rounded-[10px] flex items-center gap-2 md:gap-5`}
                style={{
                  transition: "all .3s ease-in,border .1s ease-in",
              ...
              >
                <span
                  className={`${
                    mode === "follow"
                      ? "min-w-[40px] lg:min-w-[100px] h-[40px] lg:h-[100px]"
                      : "min-w-[20px] lg:min-w-[30px] h-[20px] lg:h-[30px]"
                  } fill-black font-[none] ng-tns-c112-40`}
                  style={{
                    transition: "all .3s linear",
                  }}
                >
                ...
                <div className="">
                  ...
                  <p
                    className={`${
                      mode === "follow" ? "opacity-100" : "opacity-0 max-h-0"
                    } text-[12px] lg:text-base xl:text-[14px] font-MontserratRegular`}
                    style={{
                      transition: "all .3s linear .2s",
                    }}
                  >
                    In ‘Follow Mode,’ your account will continue following users
                    until it reaches Instagram's maximum ‘Following’ limit
                    (which is 7500). From there, interactions on our end will
                    stop and you will have to manually change your interaction
                    settings to continue experiencing results (to either
                    ‘Recommended’ or ‘Unfollow Mode’)
                  </p>
                </div>
              </div>
              ...
              >
                ...
                <div className="">
                  ...
                  <p
                    className={`${
                      mode === "unfollow" ? "opacity-100" : "opacity-0 max-h-0"
                    } text-[12px] lg:text-base font-MontserratRegular`}
                    style={{
                      transition: "all .3s linear .2s",
                    }}
                  >
                    ...
                    unfollow users that you personally followed, before or after
                    joining us. If you want to unfollow every account, please
              ...
              >
                ...
                <div className="">
                  ...
                  <p
                    className={`${
                      mode === "off" ? "opacity-100" : "opacity-0 max-h-0"
                    } text-[12px] lg:text-base font-MontserratRegular`}
                    style={{
                      transition: "all .3s linear .2s",
                    }}
                  >
                    In ‘off Mode,’ your account will off all of the users we
                    automatically followed for you. This will not off users that
                    you personally followed, before or after joining us. If you
                    want to off every account, please contact your account
                    manager.
                  </p>
                </div>
              </div>
            ...
            <div className="flex justify-center my-4">
              <button
                className="rounded-[10px] mx-auto font-MontserratSemiBold font-bold text-base py-4 w-full xl:w-[300px] h-[72px] bg-black text-white"
                onClick={(e) => {
                  e.preventDefault();
                  !loading && handleSave();
                }}
              >
                {loading ? "Saving..." : "Apply and Close"}
              </button>
            </div>
          </div>
        </div>
